...
lidar_metrics <-function(dir,chm0,chm1,year0,year1,decimated,nom_out,shp_grid)
  {
    nombre = nom_out
    dec = decimated
    y0 = year0
    ...
    acc_med2 = function(v) {
      #  mediane d'accroissement sans normalisation
      v = na.omit(v)
      med = median(v[v > 0])
      return(med)
    }
    ...
    sur_acc = function(v) {
      #  mean d'accroissement sans normalisation
      v = na.omit(v)
      acc = length(v[v > 0])
      surf = length(v)
      d = acc / surf
    ...
    delta_s <- function (test) {
      ...
      for (i in 1:length(sq_ID))
      {
        ...
        temp = data.frame(
          ...
          aff_med = aff_med(v),
          aff_mean = aff_mean(v),
          acc_mean2 = acc_mean2(v),
          aff_med2 = aff_med2(v),
          aff_mean2 = aff_mean2(v),
          aff_sum = aff_sum(v),
          acc_sum = acc_sum(v),
          absmed = absmed(v),
          absmean = absmean(v),
          ...
          not_na = not_na(v),
          sur = sur(v),
          acc_75pct = acc_75pct(v),
          acc_ab30 = acc_ab30(v),
          acc_abmin50 = acc_abmin50(v),
          acc_abmin100 = acc_abmin100(v),
          acc_abmin150 = acc_abmin150(v),
          acc_abmin300 = acc_abmin300(v)
        )
        res = rbind(res, temp)
      }
      return(res)
    ...
    norm_period_fun <-
      function(x, period) {
        (as.numeric(as.character(x))) / period
      }

    data_s <- lidar_union(delta_grid, delta_s, "square")
    data_s[, cols_period] = apply(data_s[, cols_period], 2, norm_period_fun, period =
                                    period)
    data_s$year0 = y0
    data_s$year1 = y1
    # data_s$carre=60
    ...
    data_s <-
      merge(data_s, df_shp_grid[, c("square", "Parcelle", "trait")], by = "square")
