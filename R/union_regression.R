...
# chm_rotate=FALSE
# datos=datos
# db_loop="D:/temp/data_lidar_61.csv"
# grilla="D:/CLAUDIA/PAPERS/5_Productivity/Scripts_AGB_incremental/DATA/GRILLE/Plots120x120.shp"
# resol=120

union_regression<-function(datos,db_loop,grilla,resol){
  # Preparation of the database for the two periods
  data_loop=read.table(file=db_loop,sep=",",dec=".",header=TRUE) # Wood density table
  #head(data_loop)
  ...
  for (i in 1:nrow(data_loop)){
    ...
    shp_grid=as.character(data_loop[i,8])
    ...
    union<-merge(d_sq,d[,-which(names(d) %in% c("Parcelle","trait","year0","year1"))],by="square_id")
    #head(union)

    col_y= c("AGB_G", "AGV_G", "BA_G", "AGB_D", "AGV_D", "BA_D", "agb0", "agv0", "ba0", "agb1",
             "agv1", "ba1",  "N_D", "N_R", "WD0", "WD1")

    col_x= c("acc_med", "acc_med2", "acc_mean", "aff_med", "aff_mean", "acc_mean2",
             "aff_med2", "aff_mean2", "aff_sum", "acc_sum", "cdc_med", "cdc_mean",
             "cdc_med2", "cdc_mean2","absmean","absmed","sur_aff","sur_acc")
    ...
    for (w in as.vector(unique(union$nombre))){
      ...
      for (colx_s in col_x){
        for (coly_s in col_y) {
          ...
          y_s<-data1[,coly_s]
          trait<-data1$trait.x
          db_res=regression_res(x=x_s,y=y_s,trait=as.factor(trait),nom_out=paste0(nom_out,"_",w),scatterplot=FALSE,dir_out="D:/temp/plots/reg_251120/")
          db_res$x<-colx_s
          db_res$y<-coly_s
          db_res$las<-w
          db_res$res<-resol
